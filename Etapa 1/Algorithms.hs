...
searchWithCycle :: Ord a => ([a] -> [a] -> [a]) -> a -> (S.Set a, S.Set (a, a)) -> [a] -> [a]
searchWithCycle f node graph visitedSet = generalSearch states visitedSet
    where
        states = [node]

        generalSearch states visitedSet
            | null states = reverse visitedSet
            | elem (head states) visitedSet = generalSearch (tail states) visitedSet
            | otherwise = generalSearch (f (tail states) $ S.toList $ outNeighbors (head states) graph) (head states : visitedSet)
...
search f node graph = searchWithCycle f node graph []
...
countIntermediate from to graph
    | elem to $ bfs from graph = Just (length(takeWhile (/= to) $ tail $ bfs from graph), length(takeWhile (/= to) $ tail $ dfs from graph))
    | otherwise = Nothing
