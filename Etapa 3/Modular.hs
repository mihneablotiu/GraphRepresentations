...
isModule :: Ord a
         ...
         -> Graph a
         -> Bool
isModule set graph = length (S.map f set) == 1
    where
        f = \n -> if S.filter (\n -> not (S.member n set)) (inNeighbors n graph) == standardInNeighbors
                     && S.filter (\n -> not (S.member n set)) (outNeighbors n graph) == standardOutNeighbors
                  then S.union standardInNeighbors standardOutNeighbors
                  else set

        standardInNeighbors = S.filter (\n -> not (S.member n set)) $ inNeighbors (head (S.toList set)) graph
        standardOutNeighbors = S.filter (\n -> not (S.member n set)) $ outNeighbors (head (S.toList set)) graph
...
isModularPartition :: Ord a
                   ...
                   -> Graph a
                   -> Bool
isModularPartition partition graph = length (S.map (\x -> isModule x graph) partition)
                                     == length (S.filter (== True) $ S.map (\x -> isModule x graph) partition)
...
maximalModularPartition :: Ord a
                        ...
                        -> Graph a
                        -> Partition a
maximalModularPartition partitions graph = minimumBy (\x y -> compare (length x) (length y)) (filter (\x -> length x /= 1 && isModularPartition x graph) partitions)
{-
